...
def visualize_example(example, example_index, network_output, out_folder):
    ...
    left_image_template = os.path.join(out_folder,'example_{0:04d}_image.png')
    estimated_disparity_image_template = os.path.join(out_folder,'example_{0:04d}_disparity.png')
    ground_truth_disparity_image_template = os.path.join(out_folder,'example_{0:04d}_disparity_ground_truth.png')
    nonmasked_estimated_disparity_image_template = os.path.join(out_folder,'example_{0:02d}_disparity_nomask.png')
    left_edge_template = os.path.join(out_folder,'example_{0:04d}_edge.png')

    save_image(
        filename=left_image_template.format(example_index),
        image=th.stack([example['left_image1'][0].cpu()] * 3, dim=2),
        color_first=False)
    ...
    estimated_disparity_image = network_output.cpu()

    noninf_mask = ~th.isinf(ground_truth_disparity_image)

    save_matrix(
        filename=ground_truth_disparity_image_template.format(
            example_index),
        matrix=ground_truth_disparity_image.reshape(-1, 260,346)[-1])
    save_matrix(
        filename=nonmasked_estimated_disparity_image_template.
    ...
    save_matrix(
        filename=estimated_disparity_image_template.format(
            example_index),
        matrix=estimated_disparity_image.reshape(-1, 260,346)[-1])
